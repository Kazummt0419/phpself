...
if($_SERVER["REQUEST_METHOD"] == "POST") {
  $arr_post = $_POST;
  $attend->validation_serch($arr_post);
  //バリデーションのエラーがなかった場合
  if(!isset($_SESSION['err_msg'])) {
    //検索条件を元に検索された結果をparamasに返す
    $params = $attend->serch($arr_post);
  }
}
...
elseif(isset($_GET['page_id']) || isset($_SESSION['criteria'])) {
  ...
  $params = $attend->serch($criteria_arr);
}
//URLを直接記入しページ遷移された時
...
<link rel="stylesheet" type="text/css" href="/css/serchRecord_result.css">
...
<div id="l-wrap">
  ...
  <main id="p-serchRecordResult" class="c-marginTop5 c-marginBottom5">
    <div id="p-serchRecordResult-head">
      <h2 id="p-serchRecordResult-head__title">登園記録の検索結果</h2>
    </div>
    <table id="p-serchRecordResult-form-table">
      <tr>
        <th>日付</th>
        <th>クラス名</th>
        <th>名前</th>
      ...
      <?php foreach ($params['attend_serch'] as $value): ?>
        <tr>
          <td><?php echo $value['date']; ?></td>
          <td><?php echo $value['class']; ?></td>
          <td><?php echo $value['name_child']; ?></td>
          <td><?php echo $value['temperture']; ?></td>
          <td class="c-note"><?php echo $value['message']; ?></td>
          <td class="c-note"><?php echo $value['review']; ?></td>
          <?php if($_SESSION['login_userInfo']['class'] == '教職員'): ?>
            <td><a href="serchRecord_edit.php?name_id=<?php echo $value['name_id']?>">更新</a></td>
          <?php endif; ?>
        </tr>
      <?php endforeach; ?>
    </table>
    <div id="p-serchRecordResult-table-navi" class="c-marginTop2">
